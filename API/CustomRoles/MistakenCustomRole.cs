...
namespace Mistaken.API.CustomRoles
{
    ...
    {
        /// <inheritdoc cref="CustomRole.Get(uint)"/>
        public static MistakenCustomRole Get(MistakenCustomRoles id)
            => Get((uint)id) as MistakenCustomRole;

        /// <inheritdoc cref="CustomRole.TryGet(uint, out Exiled.CustomRoles.API.Features.CustomRole)"/>
        public static bool TryGet(MistakenCustomRoles id, out MistakenCustomRole customRole)
        {
            customRole = null;
            if (!TryGet((uint)id, out var customRole2))
                return false;
